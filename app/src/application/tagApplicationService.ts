...
export default class TagApplicationService {
  ...
  public async create(name: string, bookId: string): Promise<boolean> {
    ...
    if (book.isOverNumberOfTags()) throw new OverflowDataError('本に登録できるタグの数が上限に達しています。');
    ...
    if (!isExist) {
      ...
      if (alreadyTag === null) throw new InfrastructureError('タグが見つかりませんでした。');
  ...
  public async update(id: string, name: string): Promise<void> {
    ...
    if (tag === null) throw new InfrastructureError('タグが見つかりませんでした。');
